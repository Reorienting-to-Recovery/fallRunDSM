...
harvest_adults <- function(adult_df,
  ...
  no_harvest_adults <- adult_df |>
    ...
    dplyr::group_by(watershed, origin, age) |>
    dplyr::summarise(remaining_adults = round(sum(return_total, na.rm = TRUE) * .9, 0)) # multipiles by .9 to remove 10 percent bycatch/hook mortality (high estimate)
  ...
  ocean_and_trib_harvest_adults <- adult_df |>
    ...
    dplyr::mutate(num_adults_required_after_harvest = ifelse(intelligent_crr_harvest,
           ...
           adults_after_harvest = dplyr::case_when(age == 2 ~ round((return_total *
  ...
  hatchery_adults <- dplyr::bind_rows(no_harvest_adults,
  ...
  harvested_hatchery_adults <- dplyr::bind_rows(no_harvest_adults,
                                         ocean_and_trib_harvest_adults) |>
    dplyr::filter(origin == "hatchery") |>
    dplyr::group_by(watershed, age) |>
  ...
  natural_adults <- dplyr::bind_rows(no_harvest_adults,
                              ocean_and_trib_harvest_adults) |>
  ...
  # harvested natural
  harvested_natural_adults <- dplyr::bind_rows(no_harvest_adults,
                                        ocean_and_trib_harvest_adults) |>
    dplyr::filter(origin == "natural") |>
    dplyr::group_by(watershed, age) |>
    dplyr::summarise(actual_harvest = round(sum(actual_harvest, na.rm = TRUE), 0)) |>
  ...
  total_adults <- dplyr::bind_rows(no_harvest_adults,
                            ocean_and_trib_harvest_adults) |>
    dplyr::group_by(watershed, age) |>
    dplyr::summarise(remaining_adults = round(sum(remaining_adults, na.rm = TRUE), 0)) |>
    dplyr::ungroup() |>
