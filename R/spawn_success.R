...
spawn_success <- function(escapement,
  ...
  total_nat_spawn <- tibble(watershed = fallRunDSM::watershed_labels, spawners = round(spawners * proportion_natural)) |>
    left_join(natural_age_distribution, by = c("watershed" = "watershed")) |>
  ...
  total_hatch_spawn <- tibble(watershed = fallRunDSM::watershed_labels, spawners = round(spawners * (1 - proportion_natural))) |>
    left_join(hatchery_age_distribution, by = c("watershed" = "watershed")) |>
    mutate(age_2_spawners = round(spawners * prop_2),
           age_3_spawners = round(spawners * prop_3),
           age_4_spawners = round(spawners * prop_4)) |>
    select(-c(prop_2, prop_3, prop_4, spawners, watershed)) |>
    as.matrix()
