...
baseline_fall_hatchery_release = matrix(0, nrow = 31, ncol = 4, dimnames = list(fallRunDSM::watershed_labels, c("s", "m", "l", "xl")))

# populate hatchery release as larger fish based on avg hatchery release data
baseline_fall_hatchery_release[, 3] <- summarized_release$release_number

usethis::use_data(baseline_fall_hatchery_release, overwrite = TRUE)

# hatchery releases at chipps
baseline_hatchery_releases_at_chipps <- matrix(0, nrow = 31, ncol = 4, dimnames = list(fallRunDSM::watershed_labels, fallRunDSM::size_class_labels))
usethis::use_data(baseline_hatchery_releases_at_chipps, overwrite = TRUE)
