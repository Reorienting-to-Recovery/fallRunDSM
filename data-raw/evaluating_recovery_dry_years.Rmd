---
title: "Recovery Overview - Dry Years"
date: "01/29/24"
output:
  html_document:
     code_folding: hide
...
+-----------------------+---------------------+-----------------------------------+-----------------------------------------------------------+----------------------------------------------------------------------+
| **Dry Years**      | Max habitat, Food subsidies, Predation reduction (Dry Years) | EFF (dry years) | No harvest of dry Cohort            | Terminal Hatcheries (currunt output number) |
+-----------------------+---------------------+-----------------------------------+-----------------------------------------------------------+----------------------------------------------------------------------+

## Run Dry Years Scenario

The following code block runs the Dry Years scenario.

#TODO discuss leads with tech team 
- harvest is playing a big role in the big dip - once you allow dry year in there you allow for harvest you only get a small dip and then rise again
- Also increasing delta survival gets us above 500 spawners for a consecuative 5 year period everywhere...
- The delta + outmigration survival gauntlet seems to be diminishing pops (all but tribs that are less stong get deminshed )
- All of the default calibration tribs are starting out the simulation with very few spawners seeds show limited pops on all tribs except main ones 
...
```{r}
# Dry Years spawn totals 
spawn_totals_dry_years <- dplyr::as_tibble(r2r_model_results_dy$spawners) |> #change which results to look at diff plots
...
# create spawner
spawners <- bind_rows(spawn_baseline, spawn_totals_dry_years) |> 
  pivot_wider(names_from = scenario, values_from = total_spawners) |> 
  group_by(year) |> 
  summarize(`Baseline` = sum(`Baseline`, na.rm = TRUE),
            `Dry Years` = sum(`Dry Years`, na.rm = TRUE)) |> 
  mutate(percent_difference = percent_diff(`Baseline`, `Dry Years`))
...
```{r, results='hide'}
results_df_dy <- suppressMessages(create_model_results_dataframe(r2r_model_results_dy,
                                             scenario = "Dry Years",
                                             model_parameters = fallRunDSM::r_to_r_dry_years_params, selected_run = "fall"))
...
results_df <- bind_rows(results_df_dy, results_df_baseline)
```
...
# Does not seem like it is outmigration survival 
flows <- dplyr::as_tibble(r_to_r_dry_years_params$upper_sacramento_flows) |> 
  dplyr::mutate(month = tolower(month.abb)) |>
  pivot_longer(cols = c("1980":"2000"), values_to = 'flows', names_to = "year") |> 
  mutate(date = as.Date(paste0("1", month, year), format = "%d%b%Y"),
         sac_migratory_survival = case_when(flows <= 122 ~ .03, 
                                            flows > 122 & flows <= 303 ~ .189,
                                            flows > 303 ~ .508),
         outmigration_month = ifelse(month %in% c("jan", "feb", "mar", "apr", "jun", "jul"), "Outmigration Month", "No Outmigration")) |> glimpse()

flows |>
  filter(outmigration_month == "Outmigration Month",
         year > 1990) |> 
  ggplot(aes(x = date, y = sac_migratory_survival, color = month)) + 
  geom_point() + 
  scale_color_manual(values = colors_small) +
  theme_minimal()


# Habitat capcaity plot 
spawning_months <- c(10:12)
month_lookup <- tibble(month_num = c(1:12), month = month.abb)
spawn_capacity <- r_to_r_dry_years_params$spawning_habitat |>
    as_tibble() |>
...
spawn_capacity |> 
  filter(!location %in% non_spawn_regions) |> 
  ggplot(aes(x = location, y = spawner_capacity)) +
  geom_boxplot() +
  scale_color_manual(values = colors_small) +
  theme_minimal() +
...
DSMCalibrationData::grandtab_imputed 
```
...
fish_present_data <- bind_rows(results_df_dy, results_df_baseline) |>
  filter(performance_metric == "1 All Spawners") |>
