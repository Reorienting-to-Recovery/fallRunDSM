---
title: "Recovery Overview - Habitat and Hatchery Blended Scenario (3)"
date: "01/29/24"
output:
  html_document:
     code_folding: hide
...
## Review Habitat and Hatchery Scenario

The Habitat and Hatchery scenario includes efforts in each of the 4 knobs that we can turn in the R2R process: Habitat, Hydrology, Harvest (oceans & rivers) and Hatcheries. The table below summarizes the Habitat and Hatchery scenario in comparison to the Baseline scenario.
...
+-----------------------+---------------------+-----------------------------------+-----------------------------------------------------------+----------------------------------------------------------------------+
| **Habitat and Hatchery**      | Max habitat, Food Subsidies, Predation Reduction (All years)         |  Current operations |    Current levels    | Terminal hatchery/ocean outplanting (current release output number) (All Years) |

*Additional modifications required to meet recovery: Intelligent Habitat Harvest* 

## Run Habitat and Hatchery Scenario

The following code block runs the Habitat and Hatchery scenario.
...
# seed
r2r_seeds_hh <- fallRunDSM::fall_run_model(mode = "seed",
                                           seeds = fallRunDSM::adult_seeds,
                                        ..params =  new_params, # seed with baseline, then Habitat and Hatchery
...
```{r}
# Habitat and Hatchery spawn totals 
spawn_totals_habitat_and_hatchery <- dplyr::as_tibble(r2r_model_results_hh$spawners) |> #change which results to look at diff plots
...
# create spawner
spawners <- bind_rows(spawn_baseline, spawn_totals_habitat_and_hatchery) |> 
  ...
  mutate(percent_difference = percent_diff(`Baseline`, `Habitat and Hatchery`))
...
```{r, results='hide'}
results_df_hh <- suppressMessages(create_model_results_dataframe(r2r_model_results_hh,
                                             scenario = "Habitat and Hatchery",
                                             model_parameters = fallRunDSM::r_to_r_habitat_and_hatchery_params, selected_run = "fall"))
...
results_df <- bind_rows(results_df_hh, results_df_baseline)
```
...
fish_present_data <- bind_rows(results_df_hh, results_df_baseline) |>
  filter(performance_metric == "1 All Spawners") |>
