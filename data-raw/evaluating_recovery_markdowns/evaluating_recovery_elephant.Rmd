---
title: "Recovery Overview - Elephant"
date: "09/16/2024"
output:
  html_document:
     code_folding: hide
...
## Review Elephant Scenario

The Elephant scenario includes efforts in each of the 4 knobs that we can turn in the R2R process: Habitat, Hydrology, Harvest (oceans & rivers) and Hatcheries. The table below summarizes the Elephant scenario in comparison to the Baseline scenario.
...
+-----------------------+----------------------------------------------+-------------------------+-----------------------------------------------------------+--------------------+
| **Elephant**          | Current and planned + near-future habitat    | Planned flow operations | Intelligent CRR harvest\* (in-river/Tribal, Recreational) | Phased hatcheries  |
|                       |                                              |                         |                                                           |                    |
...
## Run Elephant Scenario

The following code block runs the Elephant scenario.

```{r, echo = TRUE}
# elephant --
new_params_elephant <- R2Rscenario::load_scenario(R2Rscenario::scenarios$balanced_scenarios$elephant,
                                                  species = "fr")
new_params_elephant$movement_hypo_weights <- c(1, rep(0, 7))
...
# seed
r2r_seeds_elephant <- fallRunDSM::fall_run_model(mode = "seed",
                                           seeds = fallRunDSM::adult_seeds,
                                        ..params =  new_params, # seed with baseline, then elephant
                                        delta_surv_inflation = FALSE)

# run model
r2r_model_results_elephant <- fallRunDSM::fall_run_model(mode = "simulate",
                                                         scenario = "elephant",
                                                ..params =  new_params_elephant,
                                                seeds = r2r_seeds_elephant,
...
spawn_totals_elephant <- dplyr::as_tibble(r2r_model_results_elephant$spawners) |> #change which results to look at diff plots
...
# create spawner
spawners <- bind_rows(spawn_baseline, spawn_totals_elephant) |> 
  ...
  mutate(percent_difference = percent_diff(`Baseline`, `Elephant`))
...
```{r, results='hide'}
results_df_elephant <- suppressMessages(create_model_results_dataframe(r2r_model_results_elephant,
                                             scenario = "Elephant",
                                             model_parameters = R2Rscenario::load_scenario(R2Rscenario::scenarios$balanced_scenarios$elephant), selected_run = "fall"))
...
ind_pop_elephant <- results_df_elephant |>
  select(-size_or_age, -origin, -month, -run) |>
  filter(performance_metric %in% c("Natural Spawners", "2.2 Growth Rate Spawners",
                                   "4 PHOS", "2.1 CRR: Total Adult to Returning Natural Adult"),
         !location %in% potential_dependent_pops,
...
results_df <- bind_rows(results_df_elephant, results_df_baseline)
```
...
fish_present_data <- bind_rows(results_df_elephant, results_df_baseline) |>
  filter(performance_metric == "1 All Spawners") |>
