---
title: "Recovery Overview - Elephant Plus"
date: "`r Sys.Date()`"
output:
  html_document:
     code_folding: hide
...
## Review Elephant Plus Scenario

The Elephant Plus scenario includes efforts in each of the 4 knobs that we can turn in the R2R process: Habitat, Hydrology, Harvest (oceans & rivers) and Hatcheries. The table below summarizes the Elephant Plus scenario in comparison to the Baseline scenario.
...
+-----------------------+----------------------------------------------+-------------------------------------------+-----------------------------------------------------------+--------------------+
| **Elephant Plus**          | Current and planned + near-future habitat    | Planned flow operations + EFF (dry years) | Intelligent CRR harvest\* (in-river/Tribal, Recreational) | Phased hatcheries  |
|                       |                                              |                                           |                                                           |                    |
|                       | Floodplains (Sac)                            |                                           | Preserve tribal harvest                                   | Weirs\             |
|                       |                                              |                                           |                                                           |                    |
|                       | Food subsidies (all years, Jan-March)        |                                           | All years                                                 | All years          |
|                       |                                              |                                           |                                                           |                    |
|                       | Predation reduction (small scale, all years) |                                           |                                                           |                    |
+-----------------------+----------------------------------------------+-------------------------------------------+-----------------------------------------------------------+--------------------+

## Run Elephant Plus Scenario

The following code block runs the Elephant Plus scenario.
...
# seed
r2r_seeds_elephant_plus <- fallRunDSM::fall_run_model(mode = "seed",
                                           seeds = fallRunDSM::adult_seeds,
                                        ..params =  new_params, # seed with baseline, then elephant
                                        delta_surv_inflation = FALSE)

# run model
r2r_model_results_elephant_plus <- fallRunDSM::fall_run_model(mode = "simulate",
                                                         scenario = "elephant_plus",
                                                ..params =  new_params,
                                                seeds = r2r_seeds_elephant_plus,
                                                delta_surv_inflation = TRUE)

```

```{r}
# elephant spawn totals 
spawn_totals_elephant_plus <- dplyr::as_tibble(r2r_model_results_elephant_plus$spawners) |> #change which results to look at diff plots
...
# create spawner
spawners <- bind_rows(spawn_baseline, spawn_totals_elephant_plus) |> 
  ...
  mutate(percent_difference = percent_diff(`Baseline`, `Elephant Plus`))
...
```{r, results='hide'}
results_df_elephant_plus <- suppressMessages(create_model_results_dataframe(r2r_model_results_elephant_plus,
                                             scenario = "Elephant Plus",
                                             model_parameters = R2Rscenario::load_scenario(R2Rscenario::scenarios$balanced_scenarios$elephant_plus), selected_run = "fall"))
...
ind_pop_elephant_plus <- results_df_elephant_plus |>
  select(-size_or_age, -origin, -month, -run) |>
  filter(performance_metric %in% c("Natural Spawners", "2.2 Growth Rate Spawners",
                                   "4 PHOS", "2.1 CRR: Total Adult to Returning Natural Adult"),
         !location %in% potential_dependent_pops,
...
results_df <- bind_rows(results_df_elephant_plus, results_df_baseline)
```
...
ind_pop_elephant_plus |>
  ...
  labs(y = "",
       x = "",
       title = "Population and Hatchery Biological Parameters") +
...
fish_present_data <- bind_rows(results_df_elephant_plus, results_df_baseline) |>
  filter(performance_metric == "1 All Spawners") |>
