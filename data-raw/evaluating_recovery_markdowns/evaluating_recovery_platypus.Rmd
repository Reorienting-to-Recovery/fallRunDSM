---
title: "Recovery Overview - Platypus"
date: "09/04/2024"
output:
  html_document:
     code_folding: hide
...
## Review Platypus Scenario

The platypus scenario includes efforts in each of the 4 knobs that we can turn in the R2R process: Habitat, Hydrology, Harvest (oceans & rivers) and Hatcheries. The table below summarizes the platypus scenario in comparison to the Baseline scenario.
...
+-----------------------+---------------------+-----------------------------------+-----------------------------------------------------------+----------------------------------------------------------------------+
| **Platypus**          | Max habitat         | Ecological Functional Flows (EFF) | No harvest of dry year cohorts (ocean, in-river)          | Terminal hatchery/ocean outplanting (current release output number)\ |
|                       |                     |                                   |                                                           |                                                                      |
|                       | Food subsidies      | All years                         | Intelligent habitat harvest\* (ocean, in-river)           | All years                                                            |
|                       |                     |                                   |                                                           |                                                                      |
|                       | Predation reduction |                                   | Intelligent CRR harvest\* (in-river/Tribal, Recreational) |                                                                      |
|                       |                     |                                   |                                                           |                                                                      |
|                       | All years           |                                   | All years                                                 |                                                                      |
+-----------------------+---------------------+-----------------------------------+-----------------------------------------------------------+----------------------------------------------------------------------+

## Run platypus Scenario

The following code block runs the platypus scenario.

```{r, echo = TRUE}
# platypus --
new_params_platypus <- R2Rscenario::load_scenario(R2Rscenario::scenarios$balanced_scenarios$platypus,
                                                  species = "fr")
new_params_platypus$movement_hypo_weights <- c(1, rep(0, 7))
...
# seed
r2r_seeds_platypus <- fallRunDSM::fall_run_model(mode = "seed",
                                           seeds = fallRunDSM::adult_seeds,
                                        ..params =  new_params, # seed with baseline, then platypus
                                        delta_surv_inflation = FALSE)

# run model
r2r_model_results_platypus <- fallRunDSM::fall_run_model(mode = "simulate",
                                                         scenario = "platypus",
                                                ..params =  new_params_platypus,
                                                seeds = r2r_seeds_platypus,
                                                delta_surv_inflation = TRUE)

```

```{r}
# platypus spawn totals 
spawn_totals_platypus <- dplyr::as_tibble(r2r_model_results_platypus$spawners) |> #change which results to look at diff plots
...
# create spawner
spawners <- bind_rows(spawn_baseline, spawn_totals_platypus) |> 
  ...
  mutate(percent_difference = percent_diff(`Baseline`, `Platypus`))
...
```{r, results='hide'}
results_df_platypus <- suppressMessages(create_model_results_dataframe(r2r_model_results_platypus,
                                             scenario = "Platypus",
                                             model_parameters = R2Rscenario::load_scenario(R2Rscenario::scenarios$balanced_scenarios$platypus), selected_run = "fall"))
...
ind_pop_platypus <- results_df_platypus |>
  select(-size_or_age, -origin, -month, -run) |>
  filter(performance_metric %in% c("Natural Spawners", "2.2 Growth Rate Spawners",
                                   "4 PHOS", "2.1 CRR: Total Adult to Returning Natural Adult"),
         !location %in% potential_dependent_pops,
...
results_df <- bind_rows(results_df_platypus, results_df_baseline)
```
...
fish_present_data <- bind_rows(results_df_platypus, results_df_baseline) |>
  filter(performance_metric == "1 All Spawners") |>
