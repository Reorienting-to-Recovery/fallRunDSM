---
title: "Recovery Overview - Tortoise"
date: "09/11/2024"
output:
  html_document:
     code_folding: hide
...
+-----------------------+----------------------------------------------+-----------------------------------+--------------------------------------------------+--------------------+
| **Tortoise**          | Current + planned                            | Ecological Functional Flows (EFF) | No harvest of dry year cohorts (ocean, in-river) | Phased hatchery    |
|                       |                                              |                                   |                                                  |                    |
|                       | Floodplains / rice fields (Sac/SJ)           | Dry years                         | Harvest of hatchery fish only (ocean, in-river)  | Weirs              |
|                       |                                              |                                   |                                                  |                    |
|                       | Food subsidies (dry years)                   |                                   | Tribal harvest prioritized                       | All years          |
|                       |                                              |                                   |                                                  |                    |
|                       | Predation reduction (large-scale, dry years) |                                   | All years                                        |                    |
+-----------------------+----------------------------------------------+-----------------------------------+--------------------------------------------------+--------------------+

## Run Tortoise Scenario
...
```{r}
# Tortoise spawn totals 
spawn_totals_tortoise <- dplyr::as_tibble(r2r_model_results_tortoise$spawners) |> #change which results to look at diff plots
...
# create spawner
spawners <- bind_rows(spawn_baseline, spawn_totals_tortoise) |> 
  ...
  mutate(percent_difference = percent_diff(`Baseline`, `Tortoise`))
...
```{r, results='hide'}
results_df_tortoise <- suppressMessages(create_model_results_dataframe(r2r_model_results_tortoise,
                                             scenario = "Tortoise",
                                             model_parameters = R2Rscenario::load_scenario(R2Rscenario::scenarios$balanced_scenarios$tortoise), selected_run = "fall"))
...
ind_pop_Tortoise <- results_df_tortoise |>
  select(-size_or_age, -origin, -month, -run) |>
  filter(performance_metric %in% c("Natural Spawners", "2.2 Growth Rate Spawners",
                                   "4 PHOS", "2.1 CRR: Total Adult to Returning Natural Adult"),
         !location %in% potential_dependent_pops,
...
results_df <- bind_rows(results_df_tortoise, results_df_baseline)
```
...
fish_present_data <- bind_rows(results_df_tortoise, results_df_baseline) |>
  filter(performance_metric == "1 All Spawners") |>
