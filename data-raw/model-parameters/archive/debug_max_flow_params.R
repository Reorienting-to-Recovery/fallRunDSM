...
debug_max_flow <- list(
  ...
  spawn_decay_multiplier = DSMhabitat::spawning_decay_multiplier$run_of_river$fr,
  ...
  # DSMflow variables -----
  freeport_flows = DSMflow::freeport_flow$biop_itp_2018_2019, # ROR does mildly worse than biop here
  vernalis_flows = DSMflow::vernalis_flow$run_of_river,
  stockton_flows = DSMflow::stockton_flow$run_of_river,
  CVP_exports = DSMflow::cvp_exports$run_of_river,
  SWP_exports = DSMflow::swp_exports$run_of_river,
  proportion_diverted = DSMflow::proportion_diverted$run_of_river,
  ...
  prop_pulse_flows = DSMflow::proportion_pulse_flows$biop_itp_2018_2019, # ROR does much worse
  prop_flow_natal = DSMflow::proportion_flow_natal$run_of_river,
  upper_sacramento_flows = DSMflow::upper_sacramento_flows$biop_itp_2018_2019, # ROR does much worse
  delta_inflow = DSMflow::delta_inflow$run_of_river,
  cc_gates_days_closed = DSMflow::delta_cross_channel_closed$run_of_river["count", ], # ROR does better
  cc_gates_prop_days_closed = DSMflow::delta_cross_channel_closed$run_of_river["proportion", ], # ROR does better
  proportion_flow_bypass = DSMflow::proportion_flow_bypasses$run_of_river, # ROR does better
  gates_overtopped = DSMflow::gates_overtopped$run_of_river, # ROR does better
  ...
  degree_days = DSMtemperature::degree_days$run_of_river, # ROR temps do slightly worse
  mean_egg_temp_effect = DSMtemperature::egg_temperature_effect$fall_run,
  avg_temp = DSMtemperature::stream_temperature$run_of_river, # ROR temps do slightly worse
  ...
  inchannel_habitat_fry = DSMhabitat::fr_fry$max_flow_w_hab_projects, # vary by run # ROR does slightly worse
  inchannel_habitat_juvenile = DSMhabitat::fr_juv$max_flow_w_hab_projects, # vary by run # ROR does slightly worse
  floodplain_habitat = DSMhabitat::fr_fp$max_flow_w_hab_projects, # vary by run # ROR very similar maybe slightly better
...
usethis::use_data(debug_max_flow, overwrite = TRUE)
