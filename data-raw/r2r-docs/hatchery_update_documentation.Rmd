...
1)  Each year there are \_\_\_ hatchery adults initiated into the model based on (CWT data?)
...
1) The first two years there are \_\_\_ hatchery adults initiated into the model based on (TODO identify this?). In subsequent years hatchery adults are carried over from prior generations and not initated annually. 
2) Percent natural is output from model in the `get_spawning_adults()` submodel\
...
For the baseline Recovery scenario we use an average hatchery release value based on the average release data from all sources and years. 
...
#TODO confim okay to initiate as 1 - percent_natural - update via renaturing \# we can also add habitat constarint (assume natural get to occupy habitat first or something like that)
 - keep for now but we can think through some other logic if we see 
