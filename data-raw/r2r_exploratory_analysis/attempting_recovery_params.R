...
library(DSMhabitat)
source("data-raw/r2r_exploratory_analysis/create_wet_year_flow_data.R")
# updated params
...
recovery_params <- list(
  spawn_decay_multiplier = recovery_decay_rate,
  ...
  month_return_proportions = fallRunDSM::month_return_proportions, #this could affect prespawn survival
  ...
  # DSMflow variables -----
  freeport_flows = recovery_freeport_flows,
  vernalis_flows = recovery_vernalis_flows,
  stockton_flows = recovery_stockton_flows,
  CVP_exports = recovery_cvp_exports,
  SWP_exports = recovery_swp_exports,
  proportion_diverted = recovery_proportion_diverted,
  total_diverted = recovery_total_diverted,
  delta_proportion_diverted = recovery_delta_prop_diverted,
  delta_total_diverted = recovery_delta_total_diverted,
  prop_pulse_flows = DSMflow::proportion_pulse_flows$biop_itp_2018_2019, # same annually so leaving as is
  prop_flow_natal = recovery_prop_flow_natal,
  upper_sacramento_flows = recovery_upper_sac_flows,
  delta_inflow = recovery_delta_inflows,
  ...
  proportion_flow_bypass = recovery_bypass_flows,
  gates_overtopped = recovery_gates_overtopped,

  # DSMtemperature variables -----
  vernalis_temps = recovery_vernalis_temps, #TODO temps still exceed 18/20 in summer, could bring down
  prisoners_point_temps = recovery_pp_temps, # same here ^
  degree_days = DSMtemperature::degree_days$biop_itp_2018_2019, # stil seem hot too
  mean_egg_temp_effect = DSMtemperature::egg_temperature_effect$fall_run,
  avg_temp = recovery_stream_temps, # same here
  avg_temp_delta = recovery_delta_temps, # same here
  migratory_temperature_proportion_over_20 = DSMtemperature::migratory_temperature_proportion_over_20, # did not update, same annually

  # DSMhabitat variables -----
  spawning_habitat = recovery_spawn_hab,
  inchannel_habitat_fry = recovery_fry_hab, # vary by run
  inchannel_habitat_juvenile = recovery_juv_hab, # vary by run
  floodplain_habitat = recovery_fp_hab, # vary by run
  weeks_flooded = recovery_weeks_flooded,
  delta_habitat = recovery_delta_habitat, #TODO play around with less delta habitat (or better habitat)
  sutter_habitat = recovery_sutter_hab,
  yolo_habitat = recovery_yolo_hab,
  ...
  delta_prop_high_predation = DSMhabitat::delta_prop_high_predation, # can decrease delta predation
  ...
  prey_density = c(rep("hi", 31)), # could try increasing growth rates
  prey_density_delta = c(rep("hi", 2)), # could try increasing growth rates
...
usethis::use_data(recovery_params, overwrite = TRUE)
